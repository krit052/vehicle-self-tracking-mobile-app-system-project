...
class _AddVehicleSheetState extends State<_AddVehicleSheet> {
  ...
  final _geofenceCtrl = TextEditingController(text: '50');
  bool _saving = false;
  String? _error;
  ...
  Future<void> _pickImage(String label) async {
    try {
      ...
      if (pickedFile != null) {
        setState(() => _images[label] = File(pickedFile.path));
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      var vehicle = createRes.data as Map<String, dynamic>;
      final vehicleId = vehicle['id'] as String;

      String? photoError;
      if (_images.values.any((f) => f != null)) {
        try {
          vehicle = await _uploadVehiclePhotos(
            vehicleId: vehicleId,
            token: widget.token,
            images: _images,
          );
      ...
      if (photoError != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Vehicle added, but photo upload failed: $photoError',
            ),
            backgroundColor: AppColors.error,
          ),
        );
